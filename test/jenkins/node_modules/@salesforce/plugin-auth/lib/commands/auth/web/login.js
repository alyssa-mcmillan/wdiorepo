...
class Login extends command_1.SfdxCommand {
    async run() {
        if (this.isSFDXContainerMode()) {
            throw new core_1.SfdxError(messages.getMessage('deviceWarning'), 'DEVICE_WARNING');
        }
        if (await prompts_1.Prompts.shouldExitCommand(this.ux, this.flags.noprompt))
            return {};
        const oauthConfig = {
            loginUrl: await common_1.Common.resolveLoginUrl(ts_types_1.get(this.flags.instanceurl, 'href', null)),
        ...
        try {
            ...
            await common_1.Common.identifyPossibleScratchOrgs(fields, authInfo);
            ...
            if (error.name === 'AuthCodeExchangeError') {
                throw new core_1.SfdxError(messages.getMessage('invalidClientId', [error.message]));
            }
            throw error;
        }
    }
