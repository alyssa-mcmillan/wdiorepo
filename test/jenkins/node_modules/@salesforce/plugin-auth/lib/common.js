...
class Common {
    ...
    static async resolveLoginUrl(instanceUrl) {
        ...
        try {
            ...
            loginUrl = ts_types_1.getString(projectJson, 'sfdcLoginUrl', core_1.SfdcUrl.PRODUCTION);
        }
        catch (err) {
            const message = (ts_types_1.isObject(err) ? (_a = Reflect.get(err, 'message')) !== null && _a !== void 0 ? _a : err : err);
    ...
    static async getDevHubAuthInfos() {
        return (await Promise.all((await core_1.AuthInfo.listAllAuthFiles())
            .map((fileName) => path_1.basename(fileName, '.json'))
