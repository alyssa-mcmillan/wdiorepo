...
class LightningComponentGenerator extends sfdxGenerator_1.SfdxGenerator {
    ...
    writing() {
        ...
        if (type === 'aura') {
            this.sourceRoot(path.join(__dirname, '..', 'templates', 'lightningcomponent', 'aura', template));
            if (!internal) {
        ...
        if (type === 'lwc') {
            ...
            this.sourceRoot(path.join(__dirname, '..', 'templates', 'lightningcomponent', 'lwc', template));
            ...
            if (!internal) {
                this.fs.copyTpl(this.templatePath(`${template}.js-meta.xml`), this.destinationPath(path.join(outputdir, fileName, `${fileName}.js-meta.xml`)), { apiVersion: apiversion });
            }
        }
    }
}
