...
class Deploy extends deployCommand_1.DeployCommand {
    constructor() {
        ...
        this.isAsync = false;
        this.isRest = false;
        this.updateDeployId = kit_2.once((id) => {
    ...
    async run() {
        ...
        if (!Object.keys(this.flags).some((flag) => requiredFlags.includes(flag))) {
            throw core_1.SfdxError.create('@salesforce/plugin-source', 'deploy', 'MissingRequiredParam', [requiredFlags.join(', ')]);
        }
        await this.deploy();
        this.resolveSuccess();
        ...
        if (this.flags.validateddeployrequestid) {
            ...
            if (!this.isAsync) {
                ...
                if (!this.isJsonOutput()) {
                    const progressFormatter = kit_2.env.getBoolean('SFDX_USE_PROGRESS_BAR', true)
    ...
    resolveSuccess() {
        if (!this.isAsync) {
            const status = ts_types_1.getString(this.deployResult, 'response.status');
    ...
    async deployRecentValidation() {
        ...
        if (ts_types_1.isString(response)) {
            // SOAP API
