...
const log = core_1.Logger.childFromRoot('plugin-config:load_config_meta');
const OCLIF_META_PJSON_KEY = 'configMeta';
function loadConfigMeta(plugin) {
    var _a;
    let configMetaRequireLocation;
    try {
        const configMetaPath = ts_types_1.get(plugin, `pjson.oclif.${OCLIF_META_PJSON_KEY}`, null);
        if (typeof configMetaPath !== 'string') {
            return;
        }
        const relativePath = ts_node_1.tsPath(plugin.root, configMetaPath);
