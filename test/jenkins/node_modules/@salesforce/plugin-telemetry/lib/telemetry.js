...
class Telemetry extends kit_1.AsyncCreatable {
    ...
    static async acknowledgeDataCollection() {
        ...
        if (!Telemetry.cacheDir) {
            debuger_1.debug('Unable to check acknowledgment path because Telemetry.cacheDir is not set yet');
            return;
        }
        const acknowledgementFilePath = path_1.join(Telemetry.cacheDir, USAGE_ACKNOWLEDGEMENT_FILE_NAME);
        try {
            ...
            if (err.code === 'ENOENT') {
                ...
                await core_1.fs.mkdirp(Telemetry.cacheDir);
    ...
    getCLIId() {
        if (this.cliId)
            return this.cliId;
        const cliIdPath = path_1.join(Telemetry.cacheDir, CLI_ID_FILE_NAME);
        try {
    ...
    record(data) {
        ...
        try {
            ...
            const error = err;
            debuger_1.debug(`Error saving telemetry line to file: ${error.message}`);
        }
    }
    ...
    async clear() {
        debuger_1.debug('Deleting the log file', this.getTelemetryFilePath());
        await core_1.fs.unlink(this.getTelemetryFilePath());
    }
    async read() {
        try {
            debuger_1.debug(`Reading ${this.getTelemetryFilePath()}`);
            const data = await core_1.fs.readFile(this.getTelemetryFilePath(), 'utf8');
            const events = data
                .split(os_1.EOL)
                .filter((line) => !!line)
    ...
    upload() {
        ...
        if (!telemetryDebug) {
            debuger_1.debug(`Spawning "${nodePath} ${processPath} ${Telemetry.cacheDir} ${this.getTelemetryFilePath()}"`);
            child_process_1.spawn(nodePath, [processPath, Telemetry.cacheDir, this.getTelemetryFilePath()], {
                detached: true,
                stdio: 'ignore',
            }).unref();
        }
    ...
    async init() {
        ...
        try {
            await core_1.fs.access(Telemetry.tmpDir, core_1.fs.constants.W_OK);
        }
        catch (error) {
            const err = error;
            if (err.code === 'ENOENT') {
                debuger_1.debug('Telemetry temp dir does not exist, creating...');
                await core_1.fs.mkdirp(Telemetry.tmpDir);
            }
        }
        // Create a file descriptor to be used
        this.fileDescriptor = systemFs.openSync(this.getTelemetryFilePath(), 'a');
        debuger_1.debug(`Using telemetry logging file ${this.getTelemetryFilePath()}`);
    }
}
 ...
 */
Telemetry.tmpDir = kit_1.env.getString('SFDX_TELEMETRY_PATH', path_1.join(os_1.tmpdir(), 'sfdx-telemetry'));
Telemetry.telemetryTmpFile = path_1.join(Telemetry.tmpDir, `telemetry-${Telemetry.generateRandomId()}.log`);
