...
class SoqlQuery {
    ...
    async retrieveColumns(connection, query) {
        ...
        const results = ts_types_1.toJsonMap(await connection.request(columnUrl));
        const columns = [];
        for (let column of ts_types_1.ensureJsonArray(results.columnMetadata)) {
            ...
            if (ts_types_1.isJsonArray(column.joinColumns) && column.joinColumns.length > 0) {
                if (column.aggregate) {
                    ...
                    for (const subcolumn of column.joinColumns) {
                        const f = {
                            ...
                            name: ts_types_1.ensureString(ts_types_1.ensureJsonMap(subcolumn).columnName),
                        };
                        if (field.fields)
                            field.fields.push(f);
                    }
                    ...
                    for (const subcolumn of column.joinColumns) {
                        const f = {
                            fieldType: dataSoqlQueryTypes_1.FieldType.field,
                            name: `${name}.${ts_types_1.ensureString(ts_types_1.ensureJsonMap(subcolumn).columnName)}`,
                        };
                        columns.push(f);
                    }
                }
                ...
                const field = {
                    ...
                    name: ts_types_1.ensureString(column.displayName),
                };
