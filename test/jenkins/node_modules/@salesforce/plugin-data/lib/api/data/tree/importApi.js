...
class ImportApi {
    ...
    async import(config) {
        ...
        if (plan && this.importPlanConfig) {
            ...
            this.importPlanConfig.forEach((sobjectConfig) => {
                ...
                sobjectConfig.files.forEach((fileDef) => {
                    ...
                    if (ts_types_1.isString(fileDef)) {
                        filepath = fileDef;
                    }
                    else if (ts_types_1.isObject(fileDef)) {
                        const def = fileDef;
                        filepath = def.file;
        ...
        try {
            await executors_1.sequentialExecute(fileFns);
            await executors_1.sequentialExecute(planFns);
    ...
    createSObjectTypeMap(content, isJson) {
                ...
                Object.entries(record).forEach(([key, val]) => {
                    if (ts_types_1.isObject(val)) {
                        ...
                        if (key === 'attributes') {
                            ...
                            if (ts_types_1.isArray(v.records)) {
                                getTypes(v.records);
                            }
                        }
                    }
        ...
        if (isJson) {
            ...
            if (ts_types_1.isArray(contentJson.records)) {
    ...
    async parseSObjectTreeFile(filepath, isJson, refRegex, resolveRefs, refMap) {
        ...
        if (isJson) {
            ...
            try {
                ...
                const type = ts_types_1.getString(contentJson, 'records[0].attributes.type');
                if (type) {
    ...
    parseSObjectTreeResponse(response, filepath, isJson, saveRefs, refMap) {
        if (isJson) {
            ...
            if (ts_types_1.getNumber(response, 'results.length')) {
