...
class ExportApi {
    ...
    validate(config) {
        if (!config.query) {
            throw core_1.SfdxError.create('@salesforce/plugin-data', 'exportApi', 'queryNotProvided');
        }
        const filepath = path.resolve(process.cwd(), config.query);
        if (core_1.fs.existsSync(filepath)) {
            config.query = core_1.fs.readFileSync(filepath, 'utf8');
            if (!config.query) {
                throw core_1.SfdxError.create('@salesforce/plugin-data', 'exportApi', 'queryNotProvided');
            }
        }
        ...
        if (!config.query.startsWith('select')) {
            throw core_1.SfdxError.create('@salesforce/plugin-data', 'exportApi', 'soqlInvalid', [config.query]);
        }
        return config;
    }
    ...
    async processQueryResults(recordList) {
        ...
        const recordCount = ts_types_1.getNumber(processedRecordList, 'records.length', 0);
    ...
    async recordObjectTypes(recordList) {
        ...
        const topLevelType = ts_types_1.getString(records[0], 'attributes.type');
        ...
        records.forEach((record) => {
            Object.keys(record).map((key) => {
                ...
                if (value && ts_types_1.getNumber(value, 'records.length')) {
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
                    const firstRec = value.records[0];
                    const type = ts_types_1.getString(firstRec, 'attributes.type');
    ...
    async processRecordList(recordList, parentRef) {
        ...
        await executors_1.sequentialExecute(recordFns);
    ...
    async processRecords(parentRef, record, sobjectTree) {
        ...
        if (parentRef && this.config.plan) {
            const parentFieldName = ts_types_1.getString(parentRef, 'fieldName', '');
    ...
    generateDataPlan(sobjectTree) {
        ...
        sobjectTree.records.forEach((record) => {
            ...
            Object.keys(record).map((key) => {
                const childRecords = ts_types_1.get(record, `${key}.records`);
                if (childRecords) {
    ...
    writeDataFileSync(fileName, jsonObject) {
        ...
        if (ts_types_1.has(jsonObject, 'records') && ts_types_1.isArray(jsonObject.records)) {
