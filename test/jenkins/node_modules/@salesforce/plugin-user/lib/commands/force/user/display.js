...
class UserDisplayCommand extends command_1.SfdxCommand {
    async run() {
        ...
        if (core_1.sfdc.matchesAccessToken(this.flags.targetusername)) {
            throw new core_1.SfdxError(messages.getMessage('accessTokenError'), 'accessTokenError', [
        ...
        try {
            ...
            if (!profileName) {
                ...
                profileName = ts_types_1.getString(await conn.query(PROFILE_NAME_QUERY), 'records[0].Name');
            }
        }
        catch (err) {
            profileName = 'unknown';
            this.logger.debug(`Query for the profile name failed for username: ${username} with message: ${ts_types_1.getString(err, 'message')}`);
        }
        try {
            if (!userId) {
                const USER_QUERY = `SELECT id FROM User WHERE username='${username}'`;
                userId = ts_types_1.getString(await conn.query(USER_QUERY), 'records[0].Id');
            }
        }
        catch (err) {
            userId = 'unknown';
            this.logger.debug(`Query for the user ID failed for username: ${username} with message: ${ts_types_1.getString(err, 'message')}`);
        }
        const result = {
        ...
        if (aliasContent) {
            Object.keys(aliasContent).forEach((aliasedName) => {
                if (aliasContent[aliasedName] === username)
                    result.alias = aliasedName;
            });
        }
