...
class InstallationCancelError extends cli_1.CLIError {
    ...
    render() {
        return ` ${this.bang}   ${chalk.bold(this.message)}`;
    }
}
...
class InstallationVerification {
    ...
    async isAllowListed() {
        ...
        try {
            ...
            try {
                ...
                // send message
                process.emitWarning('support for the file `unsignedPluginWhiteList.json` will be deprecated in the future. We are transitioning towards using `unsignedPluginAllowList.json` as a replacement');
            }
    ...
    async streamTagGz() {
        ...
        try {
            await core_1.fs.mkdirp(this.getCachePath());
        }
        catch (err) {
            logger.debug(err);
        }
...
async function doPrompt(vconfig) {
    const _continue = await vconfig.prompt(chalk.dim('This plugin is not digitally signed and its authenticity cannot be verified. Continue installation y/n?'));
    ...
    try {
        ...
        if (err.name === 'NotSigned') {
            if (await verificationConfig.verifier.isAllowListed()) {
                verificationConfig.log(`The plugin [${plugin}] is not digitally signed but it is allow-listed.`);
                return;
            }
            else {
